...
public sealed class LocalAnexoFileStorage(IWebHostEnvironment environment) : IAnexoFileStorage
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (total == 0) throw new ArgumentException("Arquivo vazio nao e permitido.", nameof(conteudo));
    ...
    public Task<Stream> OpenReadAsync(string caminhoArquivo, CancellationToken cancellationToken)
    {
        ...
        if (!File.Exists(absolutePath)) throw new FileNotFoundException("Arquivo do anexo nao foi encontrado.", absolutePath);
    ...
    private string ResolveSafePath(string relativePath)
    {
        ...
        if (!absolute.StartsWith(root, StringComparison.OrdinalIgnoreCase)) throw new InvalidOperationException("Caminho de anexo invalido.");
        return absolute;
    }
