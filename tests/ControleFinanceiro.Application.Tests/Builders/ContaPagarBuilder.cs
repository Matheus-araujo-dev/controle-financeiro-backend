...
namespace ControleFinanceiro.Application.Tests.Builders;
...
public sealed class ContaPagarBuilder
{
    ...
    public ContaPagar BuildAsTracking(DbContextOptions options)
    {
        var conta = Build();
        using var context = new TestDbContext(options);
        context.ContasPagar.Add(conta);
        context.SaveChanges();
        return context.ContasPagar.First(c => c.Id == conta.Id);
    }

    public ContaPagar BuildAsNoTracking(DbContextOptions options)
    {
        var conta = Build();
        using var context = new TestDbContext(options);
        context.ContasPagar.Add(conta);
        context.SaveChanges();
        return context.ContasPagar.AsNoTracking().First(c => c.Id == conta.Id);
    }
